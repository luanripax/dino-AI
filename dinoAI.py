...
def neuralSum(w1, w2, w3, dist, vel, height):
    return (w1*dist + w2*vel + w3*height)
...
class KeySimplestClassifier(KeyClassifier):
    def __init__(self, state):
        ...
        self.populationDim = 50
        self.count = 0
        self.nWeights = self.hiddenNeurons * self.inputNumber

        data = []
        for x in range(self.populationDim):
            weights = dict()
            weightarr = []
            weights['id'] = x
            weights['points'] = -1
            for y in range(self.nWeights):
               weightarr.append(200 * np.random.rand() - 100)

            weights['values'] = weightarr
            data.append(weights)
        self.data = data

        self.currentAgent = self.data[self.count]
        self.high = 0
        self.results = []
        
    def newWeights(self):
        if(self.count < self.populationDim - 1):
            self.count += 1
        else:
            self.count = 0
            sorted_agents = sorted(self.data, key=lambda k : k['points'], reverse=True) 
            higher = sorted_agents[0]['values']
            dna = higher
            data = []
            data.append(sorted_agents[0])
            for x in range(self.populationDim):
                weights = dict()
                weightarr = []
                weights['id'] = x+1
                weights['points'] = -1
                for y in range(self.nWeights):
                    shouldMutate = np.random.rand() < self.mutationProb
                    mutationFactor = (4*np.random.rand() - 2)
                    if(shouldMutate):
                        mutation = dna[y] + mutationFactor
                    else:
                        mutation = dna[y]
                    weightarr.append(mutation)
                weights['values'] = weightarr
                data.append(weights)
            self.data = data
        self.currentAgent = self.data[self.count]

    def handleScores(self, points):
        self.currentAgent['points'] = points
 
    def keySelector(self, distance, obHeight, speed, obType):

        weights = self.currentAgent['values']
        isbird = 1 if isinstance(obType, Bird) else 0

        sum1 = neuralSum(weights[0], weights[1], weights[2], distance, speed, obHeight)
        sum2 = neuralSum(weights[3], weights[4], weights[5], distance, speed, obHeight)

        relu1 = relu(sum1)
        relu2 = relu(sum2)

        if(isbird and obHeight > 80):
            return "K_DOWN"
